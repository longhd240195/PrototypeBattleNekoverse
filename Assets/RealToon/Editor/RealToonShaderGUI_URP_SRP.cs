...
public class RealToonShaderGUI_URP_SRP : ShaderGUI
{
    ...
    string[] TOTIPS =
    {
        ...
        //Use Vertex Color As Normal Direction [8] [Outline]
        "Use vertex colors as normal direction" ,
    ...
    public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
    {
        ...
        _UVCAND = ShaderGUI.FindProperty("_UVCAND", properties);
                ...
                if (_N_F_O.floatValue == 1)
                {
                    ...
                    if (ShowOutline)
                    {
                        ...
                        if (UseSSOL == true)
                        {
                            ...
                            GUILayout.Space(10);
                            materialEditor.ShaderProperty(_UVCAND, new GUIContent(_UVCAND.displayName, TOTIPS[8]));

                            EditorGUI.BeginDisabledGroup(_UVCAND.floatValue == 1.0);
                ...
                if (ShowShadow)
                {
                    ...
                    if (_N_F_SON.floatValue == 1)
                    {
                        ...
                        if (_N_F_SON.floatValue == 1)
                        {
                            ...
                            EditorGUI.BeginDisabledGroup(_UVCAND.floatValue == 0.0);
    ...
    void CheckingPropKeyWord(Material material)
    {
        ...
        if ((material.IsKeywordEnabled("N_F_UVCAND_ON") || material.GetFloat("_UVCAND") == 1.0f))
        {
            material.EnableKeyword("N_F_UVCAND_ON");
            material.SetFloat("_UVCAND", 1.0f);
        }
        else if ((!material.IsKeywordEnabled("N_F_UVCAND_ON") || material.GetFloat("_UVCAND") == 0.0f))
        {
            material.DisableKeyword("N_F_UVCAND_ON");
            material.SetFloat("_UVCAND", 0.0f);
        }
    ...
    void REMO_OUTL()
    {
        if (remoout == true)
        {
            ChanLi("Tags{}", "Tags{" + (char)34 + "LightMode" + (char)34 + "=" + (char)34 + "remove" + (char)34 + "}", "Assets/RealToon/RealToon Shaders/Version 5/URP/Default/D_Default_URP.shader");
        ...
        {
            ChanLi("Tags{" + (char)34 + "LightMode" + (char)34 + "=" + (char)34 + "remove" + (char)34 + "}", "Tags{}", "Assets/RealToon/RealToon Shaders/Version 5/URP/Default/D_Default_URP.shader");
            ChanLi("//Cull [_DoubleSidedOutline]//OL_RCUL", "Cull [_DoubleSidedOutline]//OL_RCUL", "Assets/RealToon/RealToon Shaders/Version 5/URP/Default/D_Default_URP.shader");
    ...
    void USSOL_OR_TOL()
    {
        if (UseSSOL == true)
        {
            ...
            OLType = "Screen Space";
            UseSSOLStat = "Use Traditional Outline";
            UseSSOL = false;
            AssetDatabase.ImportAsset("Assets/RealToon/RealToon Shaders/Version 5/URP/Default/D_Default_URP.shader");
        ...
        else if (UseSSOL == false)
        {
            ...
            OLType = "Traditional";
            UseSSOLStat = "Use Screen Space Outline";
            UseSSOL = true;
            AssetDatabase.ImportAsset("Assets/RealToon/RealToon Shaders/Version 5/URP/Default/D_Default_URP.shader");
